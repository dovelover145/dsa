...
@num_points: the number of points to put in the file. Defaults to 10 if not
given
@min_value: the smallest value for an x/y coordinate. Defaults to -100 if not
given
@min_value: the largest value for an x/y coordinate. Defaults to 100 if not
given
*/
...
void write_random_shape_to_file(const char* file_name, int num_points,
                                int min_value, int max_value);
int process_command_line_parameter(const char* parameter_value,
                                   const char* parameter_name, bool has_minimum,
                                   int min_value, int default_value);
...
int main(int argc, char** argv) {
    ...
    if (argc < 2 || argc > 5) {
        printf(
            "Improper usage.\n"
            "Correct usage: %s file_path [num_points] [min_value] "
            "[max_value]\n",
            argv[0]);
    } else {
        if (argc >= 3) {
            num_points = process_command_line_parameter(argv[2], "num_points",
                                                        true, 3, 3);
        }

        if (argc >= 4) {
            min_value = process_command_line_parameter(
                argv[3], "min_value", false, default_min, default_min);
        }

        if (argc >= 5) {
            max_value = process_command_line_parameter(
                argv[4], "max_value", false, default_max, default_max);
        }
        ...
        if (max_value <= min_value) {
            ...
            printf(
                "max_value is below min_value. Using %d for min_value and %d "
                "for max_value\n",
...
void write_random_shape_to_file(const char* file_name, int num_points,
    ...
    */
    ...
    for (int i = 0; i < num_points; ++i) {
        int x_coord = rand_int(min_value, max_value),
            y_coord = rand_int(min_value, max_value);
...
int process_command_line_parameter(const char* parameter_value,
    ...
    int num_args_read =
        sscanf(parameter_value, "%d %c", &value, &should_be_blank);
    if (num_args_read == 1) {
        if (has_minimum && value >= min_value) {
            return value;
        } else {
            printf(
                "Invalid value for %s: %s. Value must be at least "
                "%d.\nDefaulting to %d\n",
