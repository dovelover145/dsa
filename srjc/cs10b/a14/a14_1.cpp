...
void sortData(vector<Highscore> &highScores);
vector<Highscore>::iterator findIndexOfLargest(vector<Highscore>::iterator startingLocation, vector<Highscore>::iterator endingLocation);
...
void sortData(vector<Highscore> &highScores) {
    for (vector<Highscore>::iterator i = highScores.begin(); i != highScores.end() - 1; i++) {
        vector<Highscore>::iterator j = findIndexOfLargest(i, highScores.end());
        swap(*j, *i);
    }
}
...
vector<Highscore>::iterator temp = i;
*i = *j;
j* = *temp;
...
vector<Highscore>::iterator findIndexOfLargest(const vector<Highscore>::iterator startingLocation, const vector<Highscore>::iterator endingLocation) {
