...
/* You should spend some time playing with these constants to try out different
   size arrays. If SIZE is big, you probably want to set PRINT_VALUES to false.
   Make sure MAX_NUMBER is big enough so you don't have a lot of repeated
   numbers in the array.  I've provided two interesting examples of how to set
   the constants.
*/
/*
...
const int SYSTEM =
    MAC;  // set this to WINDOWS or MAC in order to get the correct units
...
void print(clock_t elapsed, const int list[], int numItems) {
    ...
    std::cout << "time elapsed: " << elapsed / 1000 << "."
              << toString(elapsed % 1000);
...
bool isleaf(int k, int numItems) { return leftChild(k) >= numItems; }
...
void reheapifyDown(int list[], int numItems) {
    ...
    while (!heapOK && !isleaf(current, numItems)) {
        ...
        if (rightChild(current) < numItems &&
            list[rightChild(current)] > list[bigChildIndex]) {
